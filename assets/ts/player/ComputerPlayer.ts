...
export default class ComputerPlayer extends Player {
    ...
    public takeTurn(): boolean {
        ...
        if(pairs.length === this.hand.getCardCount()) {//game is over as the hand is empty
            this.pairs = pairs;
            return false;
        }

        if(pairs.length > 0) {//gives the UI a chance to update
            this.pairs = pairs;
            ...
            let c = this.other.surrenderCards(cards[Math.floor(Math.random() * cards.length)].getRank(), this)
            if(c !== undefined) {//other player had the requested card, check if 
